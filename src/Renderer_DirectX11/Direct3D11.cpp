...
const void Direct3D11::Start(HWND hWnd, unsigned backbufferWidth, unsigned backbufferHeight, UINT Numerator, UINT Denominator, bool fullscreen)
{
	...
	{
		throw std::runtime_error("Couldn't create DXGIFactory");
	}
	...
	if (fullscreen)
	{
		backbufferWidth = SM_CXSCREEN;
		backbufferHeight = SM_CYSCREEN;
	}
...
void Direct3D11::Shutdown( void )
{
	...
	if ( _d3dDevice )
	{
		...
		if ( references > 0 )
		{
			printf( "%d Com objects has not been released.\n" );
		}
		_d3dDevice = nullptr;
	}
...
const void Direct3D11::Resize( unsigned width, unsigned height )
{
	if ( _SwapChain )
	{
		// Unbind any buffers
	...
	unsigned sampleCount )
{
	...
	if ( height == 0 && depth == 0 )
	{
		...
		if ( FAILED( hr ) )
		{
			throw std::runtime_error( "Failed to create 1D render target texture" );
		}
	...
	else if ( depth == 0 )
	{
		...
		if ( FAILED( hr ) )
		{
			throw std::runtime_error( "Failed to create 2D render target texture" );
		}
	...
	else
	{
		...
		if ( FAILED( hr ) )
		{
			throw std::runtime_error( "Failed to create 3D render target texture" );
		}
	...
	unsigned sampleCount )
{
	...
	if ( height == 0 )
	{
		...
		if ( FAILED( hr ) )
		{
			throw std::runtime_error( "Failed to create 1D depth buffer texture" );
		}
	...
	else
	{
		...
		if ( FAILED( hr ) )
		{
			throw std::runtime_error( "Failed to create 2D depth buffer texture" );
		}
	...
	)
{
	...
	default:
		throw std::runtime_error( ("There's no implementation for resource dimension %i" + std::to_string( type)).c_str() );
	}
	...
	)
{
	...
	{
		throw std::runtime_error( "Cannot create a structured buffer with both CPU write and GPU write access" );
	}
