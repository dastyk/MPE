...
{
public:
	Direct3D11(D3D_FEATURE_LEVEL fl = D3D_FEATURE_LEVEL_11_0):_hWnd(NULL), _FeatureLevel(fl){};
	~Direct3D11() {};

	const void Start(HWND hWnd, unsigned backbufferWidth, unsigned backbufferHeight, UINT Numerator = 60, UINT Denominator = 1, bool fullscreen = false);
	void Shutdown(void);
	const void Resize(unsigned width, unsigned height);
